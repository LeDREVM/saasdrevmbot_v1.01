...
## L14 — Netlify ne proxifie PAS les websockets : le reverse-proxy `/api/*` couvre le REST, jamais le WS

- **Symptôme** : page `/markets` livrée avec un flux temps réel qui ne se connecterait jamais en prod — le client boucle en reconnexion avec backoff contre une URL `wss://saasdrevmbot.netlify.app/api/market/ws` structurellement inatteignable.
- **Cause racine** : ni les `[[redirects]]` Netlify, ni les Netlify Functions ne gèrent l'**upgrade HTTP** requis par le protocole websocket. Or `netlify.toml` route `/api/*` vers la Function `api-proxy` (cf. L13). En MODE A (`VITE_API_URL = ""`), dériver l'URL websocket de `window.location.origin` produit donc une cible qui ne peut pas aboutir — et l'échec est **silencieux côté UI** : le badge affichait juste « Flux interrompu », indiscernable d'un backend momentanément down.
- **Fix** : variable dédiée `VITE_WS_URL` visant le backend **en direct, hors Netlify** — même principe que le tunnel Cloudflare de `/bot/*`. Ordre de résolution dans `frontend/src/lib/config.js` : `VITE_WS_URL` > `API_URL` absolu > `''`. Quand elle vaut `''`, `WS_AVAILABLE` est faux et `connectMarketFeed()` **n'ouvre aucune socket** au lieu de boucler.
