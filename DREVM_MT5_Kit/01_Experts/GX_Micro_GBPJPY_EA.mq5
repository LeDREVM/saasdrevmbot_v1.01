...
input double  InpAccountSize      = 100.0;         // Solde initial (repli si pas de GlobalVariable)
...
input double  InpDailyBufferPct   = 4.0;           // Verrou perte journaliere (%)
input double  InpMaxBufferPct     = 10.0;          // Verrou perte totale (%)
input double  InpPhaseTargetPct   = 0.0;           // 0 = pas de cible globale
...
input bool    InpForceStopLoss    = true;          // SL obligatoire

//==================================================================//
//  MICRO-COMPTE : contraintes propres aux petits soldes            //
//  Sur 100 USD, le LOT MINIMUM du broker impose souvent un risque   //
//  superieur a toute gestion saine. Exemple Fusion Markets :        //
//    XAUUSD 0.01 lot = 1 USD par dollar d'or.                       //
//    Un SL realiste de 5 USD = 5% d'un compte de 100 USD.           //
...
int OnInit()
  {
   ...
   // Solde de reference : lu depuis une variable globale du terminal pour
   // survivre aux redemarrages de l'EA. InpAccountSize ne sert que de repli.
   string gvName=StringFormat("GXMICRO_BAL_%I64d_%I64d",
                              AccountInfoInteger(ACCOUNT_LOGIN),InpMagic);
   if(GlobalVariableCheck(gvName)) gInitBalance=GlobalVariableGet(gvName);
   if(gInitBalance<=0)
     {
      gInitBalance=AccountInfoDouble(ACCOUNT_BALANCE);
      if(gInitBalance<=0) gInitBalance=InpAccountSize;
      GlobalVariableSet(gvName,gInitBalance);
     }
   PrintFormat("Solde de reference : %.2f %s",gInitBalance,AccountInfoString(ACCOUNT_CURRENCY));
   ...
   if(dayId!=gCurDay && dt.hour>=InpServerDayRollover)
     {
      ...
      gDayBaseline=MathMax(bal,eq);   // le plus haut des deux
      gCurDay=dayId;
      gLockedDaily=false; gConsecLosses=0; gTradesToday=0;  // nouveau jour
     }
   if(gDayBaseline<=0) gDayBaseline=MathMax(AccountInfoDouble(ACCOUNT_BALANCE),AccountInfoDouble(ACCOUNT_EQUITY));
  }

//--- MICRO-COMPTE : scanne l'historique du jour pour compter les trades et les
//    pertes consecutives. Appele a chaque nouvelle barre (pas a chaque tick).
void ScanDayHistory()
  {
   MqlDateTime dt; TimeToStruct(TimeCurrent(),dt);
   dt.hour=InpServerDayRollover; dt.min=0; dt.sec=0;
   datetime debutJour=StructToTime(dt);
   if(TimeCurrent()<debutJour) debutJour-=86400;

   if(!HistorySelect(debutJour,TimeCurrent())) return;

   // BUG CORRIGE : avec InpUsePartial, une position genere DEUX deals de
   // sortie (partiel puis solde). L'ancien comptage voyait 2 "trades" par
   // position -> le plafond quotidien tombait apres 1,5 trade reel, et une
   // serie perdante etait interrompue par le partiel gagnant.
   // On agrege desormais le P&L PAR POSITION (DEAL_POSITION_ID).
   long   posIds[];
   double posPnl[];
   long   posTime[];
   int    nPos=0;
   ...
   for(int i=0;i<total;i++)
     {
      ...
      long tms=(long)HistoryDealGetInteger(tk,DEAL_TIME);

      int idx=-1;
      for(int k=0;k<nPos;k++) if(posIds[k]==pid){ idx=k; break; }
      if(idx<0)
        {
         nPos++;
         ArrayResize(posIds,nPos); ArrayResize(posPnl,nPos); ArrayResize(posTime,nPos);
         idx=nPos-1; posIds[idx]=pid; posPnl[idx]=0.0; posTime[idx]=tms;
        }
      posPnl[idx]+=pnl;
      if(tms>posTime[idx]) posTime[idx]=tms;   // heure de cloture definitive
     }

   // tri chronologique pour que la serie de pertes ait un sens
   for(int x=0;x<nPos-1;x++)
     for(int y=0;y<nPos-1-x;y++)
       if(posTime[y]>posTime[y+1])
         {
          long   t1=posTime[y]; posTime[y]=posTime[y+1]; posTime[y+1]=t1;
          double p1=posPnl[y];  posPnl[y] =posPnl[y+1];  posPnl[y+1] =p1;
          long   i1=posIds[y];  posIds[y] =posIds[y+1];  posIds[y+1] =i1;
         }

   int serie=0;
   for(int k=0;k<nPos;k++)
     { if(posPnl[k]<0) serie++; else serie=0; }

   gTradesToday=nPos;
   gConsecLosses=serie;
  }

//--- Verrous journaliers propres au micro-compte.
bool MicroGuardOK()
  {
   double eq=AccountInfoDouble(ACCOUNT_EQUITY);

   // 1. Objectif du jour atteint -> on arrete, on ne rend pas les gains.
   if(InpLockOnDailyTarget && InpDailyTargetPct>0 && gDayBaseline>0)
     {
      double cible=gDayBaseline*(1.0+InpDailyTargetPct/100.0);
      if(eq>=cible)
        {
         if(!gLockedDaily)
           { PrintFormat("🎯 Objectif du jour atteint (+%.1f%%) — arret des nouveaux trades.",
                         InpDailyTargetPct); gLockedDaily=true; }
         return false;
        }
     }

   // 2. Serie de pertes -> stop du jour (protection anti-tilt).
   if(InpMaxConsecLosses>0 && gConsecLosses>=InpMaxConsecLosses)
     {
      if(!gLockedDaily)
        { PrintFormat("🛑 %d pertes consecutives — arret de la journee.",gConsecLosses);
          gLockedDaily=true; }
      return false;
     }

   // 3. Plafond de trades quotidien.
...
bool RiskGuardOK()
  {
   ...
   double maxFloor=gInitBalance*(1.0-InpMaxBufferPct/100.0);
   ...
   // BUG CORRIGE : avec InpPhaseTargetPct=0, targetEq valait le solde initial
   // et le verrou se declenchait des la premiere barre -> EA muet a vie.
   // 0 signifie desormais "pas de cible globale".
   if(InpLockOnTarget && InpPhaseTargetPct>0.0)
     {
      double targetEq=gInitBalance*(1.0+InpPhaseTargetPct/100.0);
      if(eq>=targetEq){ if(!gLockedTarget){ Print("🎯 Cible atteinte — stop nouveaux trades."); gLockedTarget=true; } }
...
//--- Perte en devise du compte pour un lot donne, SL a `dist` du prix.
double LossForLot(double lot,double dist)
  {
   double tv=SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_VALUE);
   double ts=SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE);
   if(dist<=0||tv<=0||ts<=0) return 0.0;
   return (dist/ts)*tv*lot;
  }

//--- MICRO-COMPTE : renvoie 0 si aucun volume tradable ne respecte le risque.
//    Sur un compte de 100 USD, le lot minimum impose souvent un risque
//    superieur a InpHardMaxRiskPct. Plutot que d'arrondir vers le haut et de
//    sur-risquer en silence, on REFUSE le trade et on l'explique.
double CalcLot(double entry,double sl)
  {
   double bal=AccountInfoDouble(ACCOUNT_BALANCE);
   double riskAmount=bal*InpRiskPercent/100.0;
   double hardCap   =bal*InpHardMaxRiskPct/100.0;
   double dist=MathAbs(entry-sl);
   double minLot=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN);

   if(dist<=0) return 0.0;
   double lossPerLot=LossForLot(1.0,dist);
   if(lossPerLot<=0) return 0.0;

   double lot=NormalizeLot(riskAmount/lossPerLot);

   // Le volume calcule est descendu sous le minimum broker : on ne peut pas
   // trader plus petit. On verifie ce que couterait reellement le lot minimum.
   if(lot<minLot)
     {
      double perteMini=LossForLot(minLot,dist);
      if(perteMini>hardCap)
        {
   ...
   // Securite finale : jamais au-dessus du plafond dur.
   if(LossForLot(lot,dist)>hardCap && InpRefuseIfLotTooBig)
     {
      PrintFormat("⛔ TRADE REFUSE | risque %.2f%% > plafond %.1f%%.",
                  LossForLot(lot,dist)/bal*100.0,InpHardMaxRiskPct);
      return 0.0;
     }
   if(InpMaxLotCap>0 && lot>InpMaxLotCap) lot=NormalizeLot(InpMaxLotCap);
   return lot;
  }
...
void FireSignal(string grade)
  {
   ...
   if(lot<=0)
     {
      // Refus documente : le signal etait valide, c'est le compte qui est trop
      // petit pour cet instrument. Utile a tracer pour choisir ses symboles.
      PrintFormat("Signal %s ignore : volume incompatible avec le risque autorise.",_Symbol);
      return;
     }
